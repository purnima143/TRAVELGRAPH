onload = function () {
    ...
    //edges.style['-webkit-text-stroke'] = 'none';
    ...
    function createData() {
        ...
        for (let i = 2; i <= V; i++) {
            ...
            edges.push({ type: 0, from: i, to: neigh, color: 'blue', label: String(Math.floor(Math.random() * 70) + 31) });
        }
    ...
    genNew.onclick = function () {
        ...
        temptext2.innerText = 'Best path from ' + cities[src - 1] + ' to ' + cities[dst - 1];
    ...
    function pushEdges(dist, curr, reverse) {
        ...
        while (dist[curr][0] !== 0) {
            let fm = dist[curr][1];
            if (reverse)
                tmp_edges.push({ arrows: { to: { enabled: true } }, from: curr + 1, to: fm + 1, color: 'blue', label: String(dist[curr][0] - dist[fm][0]) });
            else
                tmp_edges.push({ arrows: { to: { enabled: true } }, from: fm + 1, to: curr + 1, color: 'blue', label: String(dist[curr][0] - dist[fm][0]) });
            curr = fm;
        }
        return tmp_edges;
    }
